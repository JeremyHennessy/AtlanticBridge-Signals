...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.ABOpportunities=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  const checkLabels={legal_identity:'Legal identity',corporate_group:'Corporate group',civilian_scope:'Civilian scope',canadian_relevance:'Canadian relevance',current_status:'Current status',source_reuse:'Source reuse'};
  function readiness(decision){
    const states=Object.fromEntries(checks.map(k=>[k,decision?.checks?.[k]||'UNKNOWN']));
    const blockers=checks.filter(k=>states[k]!=='SUPPORTED');
    return {states,supported:checks.length-blockers.length,total:checks.length,blockers,contradicted:blockers.filter(k=>states[k]==='CONTRADICTED')};
  }
  ...
  return {validate,qualified,queue,select,checks,checkLabels,readiness,latestEvidence,reviewGroups};
});
...
function renderTodaySummary(){
  ...
  listNode.innerHTML=rows.length?rows.map(r=>`<article class="company-notice" data-today-opportunity="${escapeHtml(r.id)}"><div class="work-row-heading"><h3><a class="company-dossier-link" href="${companyHash(r.company_id)}">${escapeHtml(r.company_name)}</a></h3><span class="status-chip ${r.qualified?"status-establishment":"status-unresolved"}">${r.qualified?"Qualified for investigation":"Needs review"}</span></div><p><strong>${escapeHtml(r.title)}</strong> · ${escapeHtml(r.location)}</p><p class="small muted">${escapeHtml(r.kind)} · Public document: ${escapeHtml(formatDate(r.day))}</p><p class="small"><strong>Qualification:</strong> ${ABOpportunities.readiness(r.decision).supported}/${ABOpportunities.checks.length} supported · ${ABOpportunities.readiness(r.decision).blockers.length?`Needs ${ABOpportunities.readiness(r.decision).blockers.slice(0,3).map(k=>ABOpportunities.checkLabels[k]).join(', ')}${ABOpportunities.readiness(r.decision).blockers.length>3?' + more':''}`:'All gates supported'}</p><p><strong>Next action:</strong> ${escapeHtml(r.next_action)}</p><div class="work-actions"><a class="button" href="${companyHash(r.company_id)}">Open dossier →</a><a class="source-link" href="${escapeHtml(r.source_url)}" target="_blank" rel="noopener noreferrer">Original source ↗</a></div></article>`).join(""):`<div class="empty-state"><h3>${model.source_available||model.history_available?"No current review items in the loaded 90-day window.":"Current review coverage is unavailable."}</h3><p>Missing source coverage remains unknown. Historical cases and saved work are still available.</p></div>`;
}
function renderOpportunityQueue(params=new URLSearchParams(location.hash.split('?')[1]||'')){
  const model=ABOpportunities.queue(state.live,reviewedCatalog,opportunityRegister),view=['qualified','history'].includes(params.get('view'))?params.get('view'):'current',query=params.get('q')||'',blocker=ABOpportunities.checks.includes(params.get('blocker'))?params.get('blocker'):'';
  let rows=ABOpportunities.select(model,view,query);
  if(blocker)rows=rows.filter(r=>ABOpportunities.readiness(r.decision).blockers.includes(blocker));
  $('opportunity-search').value=query;
  $('opportunity-blocker').value=blocker;
  ...
  const gateCounts=Object.fromEntries(ABOpportunities.checks.map(k=>[k,model.current.filter(r=>ABOpportunities.readiness(r.decision).blockers.includes(k)).length]));
  $('opportunity-gates').textContent='Current queue blockers · '+ABOpportunities.checks.map(k=>`${ABOpportunities.checkLabels[k]} ${gateCounts[k]}`).join(' · ');
