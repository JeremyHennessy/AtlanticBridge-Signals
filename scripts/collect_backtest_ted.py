...
def collect(
    *,
    ...
    for entity in entities:
        ...
        alias_queries: list[dict[str, object]] = []
        ...
        for alias in aliases:
            ...
            complete = len(result.notices) == result.total_notice_count
            if not complete:
            ...
            for notice in result.notices:
                mention_names = [
                    mention.winner_name for mention in notice.winner_mentions()
                ]
                mention_normalized = {
                ...
                if parsed < coverage_start or parsed > end_date:
                    raise ValueError(
                        "TED result outside requested coverage window: "
                ...
                if row is None:
                    ...
                    row = {
                        ...
                        "publicly_available_date_basis": (
                            ...
                            "winner-name candidate query and exact-normalized "
                            "against a reviewed foreign legal-entity alias."
                        ),
    ...
    return {
        ...
        "signal_definition": (
            ...
            "the event-time cutoff where a returned winner-name exactly normalizes "
            "to a reviewed foreign legal-entity alias."
        ),
        "coverage_definition": (
            "Each reviewed alias is used in TED's winner-name query as a candidate "
            "retrieval superset over the complete bounded backtest window. Every "
            "returned winner-name is post-filtered to exact normalized equality. "
            "Absence is only absence of that narrow exact-alias winner signal after "
...
def validate_against_manifest(
    ...
    proof = manifest["proof_summary"]
    assert isinstance(summary, dict)
    assert isinstance(proof, dict)

    expected_summary = {
        "entity_count": proof["entity_count"],
        "entity_alias_query_count": proof["normalized_alias_query_count"],
        "complete_entity_query_sets": proof["complete_entity_query_sets"],
        "evidence_record_count": proof["exact_reviewed_alias_evidence_records"],
        "entities_with_evidence": proof["entities_with_exact_evidence"],
    }
    if summary != expected_summary:
        raise ValueError(
            f"TED historical proof summary drifted: {summary} != {expected_summary}"
        )

    query_hash = hashlib.sha256(
        canonical_json(payload["query_proof"]).encode("utf-8")
    ).hexdigest()
    if query_hash != proof["query_proof_canonical_sha256"]:
        raise ValueError(
            "TED historical query proof drifted: "
            f"{query_hash} != {proof['query_proof_canonical_sha256']}"
        )

    coverage_hash = hashlib.sha256(
        canonical_json(payload["coverage"]).encode("utf-8")
    ).hexdigest()
    if coverage_hash != proof["coverage_canonical_sha256"]:
        raise ValueError(
            "TED historical coverage drifted: "
            f"{coverage_hash} != {proof['coverage_canonical_sha256']}"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--manifest",
        default="reviews/backtests/2026-09-21-ted-historical-manifest.json",
    )
    parser.add_argument(
        "--output",
        default="control-output/ted-signal-evidence.json",
    )
    ...
    validate_against_manifest(payload, manifest)
